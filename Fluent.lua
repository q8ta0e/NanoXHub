	...
	return function(Config)
		...
		if Config.UserInfo then
			...
			local UserInfoSection = New("Frame", {
				...
				Position = Config.UserInfoTop and UDim2.fromOffset(0, 0) or UDim2.new(0, 0, 1, -(userInfoHeight + 4)),
				ZIndex = 15,
				Parent = TabFrame,
			})
			...
			New("Frame", {
				...
				BackgroundTransparency = 0.5,
				Size = UDim2.new(1, 0, 0, 1),
				Position = Config.UserInfoTop and UDim2.fromOffset(0, userInfoHeight) or UDim2.new(0, 0, 1, -(userInfoHeight + 4)),
				ZIndex = 15,
				Parent = TabFrame,
				ThemeTag = {
					BackgroundColor3 = "TitleBarLine",
...
ElementsTable.Dropdown = (function()
	...
	function Element:New(Idx, Config)
		...
		local function RecalculateListSize()
			...
			DropdownHolderCanvas.Size = UDim2.fromOffset(canvasWidth, 360)
			
			local many = visibleCount > 10
			DropdownHolderFrame.Size = UDim2.fromScale(1, many and (360 / math.max(360, 1)) or 1)
		end

		local function RecalculateListSizeDefault()
			if not DropdownHolderCanvas or not DropdownHolderFrame then return end
